...
const AllNotes = async () => {
    ...
    const currentDate = formateDate(new Date().toISOString());

  return (
    <div className='bg-white py-6 px-6 dark:bg-secondary'>
        <div className='grid grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1 gap-5 justify-center mt-20 max-lg:mt-5'>
            ...
            {notes.length === 0 ? <h1>You Don't have any saved note</h1> : (
                notes.map(({_id, title, content, date})=>(
                    <Link  href={`/notes/${_id}`}>
                        <div key={_id} className='relative bg-[#FCFCFD] dark:bg-[#222530] dark:border-[#222530] dark:text-white border-2 border-[#E4E7EC] flex flex-col h-[250px]  max-sm:w-full rounded-md'>
